...
class ExtractorTest < Minitest::Test
  ...
  def test_skips_claim_already_represented_by_an_accepted_entry
    build_memory_project do |root|
      transcript_path = write_transcript(root, [["user", "Use detached hooks"], ["assistant", "Agreed"]])
      fingerprint = Cyborg::MemoryCandidates::Candidate.fingerprint(decision_candidate)
  ...
  def extractor(root, backend)
    Cyborg::MemoryCandidates::Extractor.new(
      project_root: root,
      backend: backend,
      now: -> { @now },
      timeout_seconds: 7
  ...
  def decision_candidate
    {
      ...
      "tags" => ["memory", "hooks"],
      "components" => ["bin/enqueue-memory-candidates", "docs/memory"],
  ...
  def learning_candidate
    {
      ...
      "components" => ["adapters/codex/session-end"],
