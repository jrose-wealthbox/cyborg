...
4. Correct the entry content if needed, then run `bin/manage-memory-candidate promote <ID>`. Promotion moves it to `decisions/` or `learnings/`, assigns a canonical dated ID, sets `status: active`, records `promoted_from`, and updates the index.
...
```sh
bin/manage-memory-candidate dismiss <ID> "<reason>"
```
