...
module Cyborg
  module MemoryCandidates
    module Configuration
      ...
      def enabled?(env)
        !%w[0 false no off].include?(env.fetch("CYBORG_MEMORY_CANDIDATES_ENABLED", "true").downcase)
      end

      def timeout_seconds(env)
        bounded_integer(env["CYBORG_MEMORY_CANDIDATE_TIMEOUT_SECONDS"], default: 20, min: 1, max: 60)
      end

      def max_transcript_bytes(env)
        bounded_integer(env["CYBORG_MEMORY_CANDIDATE_MAX_BYTES"], default: TranscriptReader::DEFAULT_MAX_BYTES, min: 4096, max: 1024 * 1024)
      end

      def max_messages(env)
        bounded_integer(env["CYBORG_MEMORY_CANDIDATE_MAX_MESSAGES"], default: TranscriptReader::DEFAULT_MAX_MESSAGES, min: 10, max: 500)
      end
    ...
    class EnqueueCLI
      ...
      def default_queue(event)
        ...
        queue_dir = @env["CYBORG_MEMORY_CANDIDATE_QUEUE_DIR"] || File.join(Dir.tmpdir, "cyborg-memory-candidates", root_digest)
        worker_path = @env["CYBORG_MEMORY_CANDIDATE_WORKER"] || File.expand_path("../../../bin/memory-candidate-worker", __dir__)
