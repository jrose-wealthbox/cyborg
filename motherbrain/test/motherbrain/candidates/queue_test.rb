...
class QueueTest < Minitest::Test
  def test_enqueues_idempotently_and_dispatches_without_running_worker_inline
    Dir.mktmpdir do |queue_dir|
      ...
      first = Motherbrain::Candidates::Queue.new(
        queue_dir: queue_dir,
        worker_path: "/project/motherbrain/bin/memory-candidate-worker",
        spawner: spawner
      ).enqueue(event)
      second = Motherbrain::Candidates::Queue.new(
        queue_dir: queue_dir,
        worker_path: "/project/motherbrain/bin/memory-candidate-worker",
        spawner: spawner
      ...
      assert_equal ["/project/motherbrain/bin/memory-candidate-worker", first], calls.first
    end
  end
  ...
  def test_returns_nil_when_queue_cannot_be_written
    Dir.mktmpdir do |tmpdir|
      ...
      result = Motherbrain::Candidates::Queue.new(
