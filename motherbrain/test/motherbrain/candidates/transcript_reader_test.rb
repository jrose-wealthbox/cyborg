...
class TranscriptReaderTest < Minitest::Test
  def test_reads_claude_and_codex_messages_while_ignoring_tools_and_instructions
    Dir.mktmpdir do |tmpdir|
      ...
      transcript = Motherbrain::Candidates::TranscriptReader.new(max_bytes: 10_000, max_messages: 10).read(path)
  ...
  def test_bounds_input_to_the_tail_and_message_limit
    Dir.mktmpdir do |tmpdir|
      ...
      transcript = Motherbrain::Candidates::TranscriptReader.new(max_bytes: 500, max_messages: 2).read(path)
  ...
  def test_returns_nil_for_missing_or_malformed_transcript
    reader = Motherbrain::Candidates::TranscriptReader.new
