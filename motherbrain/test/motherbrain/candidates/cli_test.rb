...
class CliTest < Minitest::Test
  ...
  def test_enqueue_cli_normalizes_provider_input
    ...
    status = Motherbrain::Candidates::EnqueueCLI.new(
      harness: "codex",
  ...
  def test_enqueue_cli_fails_open_when_disabled_or_input_is_malformed
    ...
    disabled = Motherbrain::Candidates::EnqueueCLI.new(
      harness: "claude_code",
      stdin: StringIO.new("{}"),
      env: {"MOTHERBRAIN_CANDIDATES_ENABLED" => "false"},
      queue: queue
    ).run
    malformed = Motherbrain::Candidates::EnqueueCLI.new(
      harness: "claude_code",
  ...
  def test_extract_cli_accepts_normalized_json_and_reports_zero_without_backend
    build_memory_project do |root|
      ...
      status = Motherbrain::Candidates::ExtractCLI.new(
  ...
  def test_default_queue_spawns_worker_without_waiting_for_it
    Dir.mktmpdir do |tmpdir|
      ...
      queue = Motherbrain::Candidates::Queue.new(queue_dir: File.join(tmpdir, "queue"), worker_path: worker)
