...
```sh
export MOTHERBRAIN_CANDIDATE_BACKEND='/absolute/path/to/your-memory-extractor'
```
...
| --- | --- | --- |
| `MOTHERBRAIN_CANDIDATES_ENABLED` | `true` | Set to `false`, `0`, `no`, or `off` to disable. |
| `MOTHERBRAIN_CANDIDATE_TIMEOUT_SECONDS` | `20` | Analysis timeout, clamped to 1–60 seconds. |
| `MOTHERBRAIN_CANDIDATE_MAX_BYTES` | `262144` | Transcript tail, clamped to 4 KiB–1 MiB. |
| `MOTHERBRAIN_CANDIDATE_MAX_MESSAGES` | `200` | User/assistant messages, clamped to 10–500. |
| `MOTHERBRAIN_CANDIDATE_QUEUE_DIR` | OS temporary directory | Override only for diagnostics or tests. |
    ...
    "SessionEnd": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "${CLAUDE_PROJECT_DIR}/motherbrain/adapters/claude-code/session-end",
            "timeout": 2
          }
        ]
      }
    ...
    "SessionEnd": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "/absolute/path/to/cyborg/motherbrain/adapters/codex/session-end",
            "timeout": 2
          }
        ]
      }
...
`motherbrain/bin/extract-memory-candidates` accepts one normalized event on standard input and always emits JSON containing `candidate_ids`:
...
motherbrain/bin/manage-memory-candidate dismiss LRN-CAND-<digest> "Duplicate of LRN-..."
